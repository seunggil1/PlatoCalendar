...
class Onestop {
  ...
  static Future<bool> loginDeprecated() async {
    ...
    try {
      ...
      // id pw 체크
      list.forEach((iter) {
        if (iter.attributes['name'] == "reTry") if (iter.attributes['value'] ==
            "Y") {
  ...
  static Future<bool> getTestTimeTable() async {
    try {
      ...
      response = await http.get(
          Uri.parse(
              'https://e-onestop.pusan.ac.kr/menu/class/C06/C06002?menuId=2000030702&rMenu=03'),
          headers: {
            'Host': 'e-onestop.pusan.ac.kr',
            'Connection': 'keep-alive',
            'Cache-Control': 'no-cache',
      ...
      String userInfo =
          response.body.substring(index, response.body.indexOf(';', index));
      userInfo =
          userInfo.substring(userInfo.indexOf('\'') + 1, userInfo.length - 1);

      // 학부 대학원 구분
      index = response.body.indexOf('year, term,');
      String studentType = response.body
          .substring(index + 12, response.body.indexOf('testgbn', index));
      index = studentType.indexOf(',');
      studentType = studentType.substring(
          index + 3, studentType.indexOf(',', index + 3) - 1);
      ...
      response = await http.post(
          Uri.parse(
              'https://e-onestop.pusan.ac.kr/middleware/study/testTimeTable/testTimeTableCheck'),
          headers: {
            'Host': 'e-onestop.pusan.ac.kr',
            'Connection': 'keep-alive',
            'Cache-Control': 'no-cache',
      ...
      await testTimeParser(jsonDecode(response.body)["dataset1"],
          [now.year.toString(), semester, test == "10" ? "중간고사" : "기말고사"]);
    } catch (e) {
      return false;
    }
  ...
  static Future<bool> logout() async {
    try {
      http.Response response;
      response = await http
          .get(Uri.parse('https://e-onestop.pusan.ac.kr/sso/logout'), headers: {
        "Cookie": "$jsessionid",
        "Host": "e-onestop.pusan.ac.kr",
        "Referer": "https://e-onestop.pusan.ac.kr/index?home=home",
      });
      ...
      response = await http.post(
          Uri.parse('http://sso.pusan.ac.kr/isignplus/logout.jsp'),
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
            "Host": "sso.pusan.ac.kr",
          },
      ...
      response =
          await http.post(Uri.parse('http://sso.pusan.ac.kr/LoginServlet'),
              headers: {
                "Content-Type": "application/x-www-form-urlencoded",
